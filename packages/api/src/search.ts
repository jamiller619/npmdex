...
async function searchRelevance(
  ...
  const tagEscaped = queryLower.replace(
    /[\\{}()|<>\-~[\]"':;.,/^$*=&#?`+%!@]/g,
    '\\$&',
  )
...
export async function searchPackages(
  ...
  const packages: PackageResult[] = results.documents.map(
    (doc: { id: string; value: Record<string, unknown> }) =>
      parseDocument(doc.value),
  )
