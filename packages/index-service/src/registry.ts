...
export async function fetchPackageInfo(name: string): Promise<RegistryPackageInfo | null> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch package ${name}: ${res.status} ${res.statusText}`);
  }
